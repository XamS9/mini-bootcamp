...
import CreateUserApi from "../apis/createUser.api";
...
function CreateUser() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (e.status) {
      await CreateUserApi(e, user, password2).then((res) => {
        if(res.ok) setUser(initialState); setPassword2("");
      });
    }
  };
  ...
  return (
    <div className="d-flex align-items-center justify-content-center mt-2 p-2">
      <div className="w-75">
        <h2 className="form-group mb-2">Create user</h2>
