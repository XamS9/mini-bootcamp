...
function NewSubcategory() {
  ...
  const titles = ["Name", "Description", "Category"];
  const handleSubmit = async (e) => {
    await formsValidator(e);
    if (e.status) {
      await PostJSONData(
        "/subcategories/" + document.getElementById("dependid").value,
        data
      ).then((res) => {
        if (res.ok) {
  ...
  return (
    <div className="d-flex align-items-center justify-content-center">
      <div className="w-75">
        <h2 className="form-group mb-2">Create subcategory</h2>
        <form
          id="form"
              ...
              <div className="form-group mb-2" key={index}>
                ...
                {index === 2 ? (
                  <select
                    ...
                    required
                    onClick={(e) =>
                      GetData("/categories").then((data) => setDepends(data))
                    }
                  >
